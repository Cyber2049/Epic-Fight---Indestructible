...
public class StatusIndicator extends EntityIndicator {
    ...
    public boolean shouldDraw(LivingEntity entityIn, @Nullable LivingEntityPatch<?> entitypatch, LocalPlayerPatch playerpatch) {
        ...
        } else if (entitypatch instanceof IBossEventCapability ibc && ibc.hasBossBar()){
            return false;
        }
        ...
        return (!entityIn.getActiveEffects().isEmpty() || entityIn.getHealth() < entityIn.getMaxHealth() || (entitypatch instanceof IAdvancedCapability iac && iac.getStamina() < iac.getMaxStamina())) && entityIn.deathTime < 19;
    }
    ...
    public void drawIndicator(LivingEntity entityIn, @Nullable LivingEntityPatch<?> entitypatch, LocalPlayerPatch playerpatch, PoseStack matStackIn, MultiBufferSource bufferIn, float partialTicks) {
        boolean adjustUI = entitypatch instanceof IAdvancedCapability && UIConfig.REPLACE_UI.get();
        ...
        if(entitypatch instanceof IAdvancedCapability iac){
            this.renderStamina(iac, mvMatrix, vertexBuilder);
        }
    }


    private void renderStamina(IAdvancedCapability iac, Matrix4f mvMatrix, VertexConsumer vertexBuilder) {
