...
public class BossBarGUi {
    ...
    public void renderBossBar(CustomizeGuiOverlayEvent.BossEventProgress event){
        ...
        BossBarEntities.values().forEach((k)->{
            if(event.getBossEvent().getName().getString().contains(k.getOriginalName())){
                event.setCanceled(true);
                event.setIncrement(0);
            }
        });
        ...
        if(info != null){
            ...
            int l = Minecraft.getInstance().font.width(info.getDisplayName());
            ...
            float healthRatio = Mth.clamp(info.getHealthRatio(),0.0F,1.0F);
            int health = (int) (256 * healthRatio);
            float staminaRatio = Mth.clamp(info.getStaminaRatio(), 0.0F, 1.0F);
            int stamina = (int) (256 * staminaRatio);
            guiGraphics.blit(rl, x, y, 0, 0, 256, 19,255,255);
            guiGraphics.blit(rl, x, y, 0, 21, health, 19,255,255);
            guiGraphics.blit(rl, x, y + 18, 0, 42, 256, 10,255,255);
            guiGraphics.blit(rl, x, y + 18, 0, 55, stamina, 10,255,255);
            guiGraphics.blit(rl, x, y, 0, 68, 256, 29,255,255);
            guiGraphics.drawString(Minecraft.getInstance().font , info.getDisplayName(), i1, y-9, 16777215);
